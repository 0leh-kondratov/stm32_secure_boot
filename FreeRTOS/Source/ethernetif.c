...
void HAL_ETH_RxLinkCallback(void **pStart, void **pEnd, uint8_t *buff, uint16_t Length)
{
  ...
  *ppEnd  = p;
  ...
  /* Invalidate D-Cache for the RX buffer: ETH DMA wrote to physical RAM; the cache may still hold
   * stale data. SCB_InvalidateDCache_by_Addr ensures the CPU sees the DMA-written bytes. Required
   * when the buffer lies in a cacheable region; if MPU marks the region non-cacheable, this is redundant
   * but harmless. */
