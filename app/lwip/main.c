/**
  ******************************************************************************
  * @file    main.c
  * @brief   Minimal and stable LwIP + FreeRTOS startup (NUCLEO-H743ZI2).
  ******************************************************************************
  */
...
int main(void)
{
  ...
  if (StartDefaultTaskHandle == NULL)
  {
    log_puts("lwip: ERROR osThreadNew(Start) failed\r\n");
  }

  osKernelStart();

  log_puts("lwip: ERROR osKernelStart returned\r\n");
  for (;;)
  {
  }
}
...
static void StartDefaultTask(void *argument)
{
  ...
  log_puts("lwip: Start task\r\n");

  tcpip_init(NULL, NULL);
  log_puts("lwip: tcpip_init ok\r\n");

  Netif_Config();
  log_puts("lwip: netif configured\r\n");
...
static void MPU_Config(void)
{
  ...
  /* Default deny, then enable only required regions. */
  ...
  /* ETH descriptors + RX pool in D2 SRAM: non-cacheable area. */
  MPU_InitStruct.Enable = MPU_REGION_ENABLE;
  ...
  /* LwIP heap in D2 SRAM: non-cacheable shared region. */
  MPU_InitStruct.Enable = MPU_REGION_ENABLE;
